...
Application::Application(bool worldBuilder)
	: mWindow(NULL), mRenderer(NULL), mBoardTexture(NULL), mCardBackTexture(NULL),
	  ...
	  mWorldBuilderSelectedObjectTemplate(-1), mWorldBuilderSelectedPortal(-1),
	  mWorldBuilderPortalEndpoint(0), mWorldBuilderPortalCreating(false),
	  mWorldBuilderSelectedRegion(-1), mWorldBuilderRegionPlacementMode(0),
	  ...
	  mWorldBuilderErasing(false), mWorldBuilderLastBrushX(-1),

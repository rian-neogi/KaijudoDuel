...
namespace
{
		...
		const std::string& committedAction, bool usedFallback)
	{
		...
		output << std::fixed << std::setprecision(3)
			<< "AI MCTS: rollouts=" << result.iterationsCompleted
	...
	bool chooseAutomaticAction = false;
	{
		...
		if (mAiSearch != NULL && mAiSearch->isActive())
		{
			if (mAiSearch->isFinished())
			{
				...
				{
					decision.source = AiDecisionSource::Mcts;
...
bool Application::findDragAction(const std::string& type, int cardId, int targetId, Message& result)
{
	...
	for (size_t i = 0; i < moves.size(); ++i)
	{
		...
		if (type == "cardplay" || type == "cardmana")
		{
			...
			if (type == "cardplay" && targetId >= 0 &&
				messageInt(moves[i], "evobait") != targetId &&
				messageInt(moves[i], "evobait2") != targetId) continue;
			if (type == "cardplay" && targetId < 0 &&
				(messageInt(moves[i], "evobait") >= 0 ||
				 messageInt(moves[i], "evobait2") >= 0)) continue;
		}
		else if (type == "creatureattack")
		{
...
std::string Application::actionLabel(const Message& message) const
{
	...
	if (type == "cardplay")
	{
		...
		if (bait2 >= 0)
			return "Evolve " + cardName(messageInt(message, "card")) + " on " +
				cardName(bait) + " + " + cardName(bait2);
		if (bait >= 0)
			return "Evolve " + cardName(messageInt(message, "card")) + " on " + cardName(bait);
		return "Cast " + cardName(messageInt(message, "card"));
	}

...
def parser():
    ...
    c=commands.add_parser("inspect",help="Bounded ASCII walkability and occupant map");select(c)
    c=commands.add_parser("cell",help="Explain layers, collision, occupants and tags at a cell")
    c.add_argument("--map",default="overworld");c.add_argument("--at",nargs=2,type=int,required=True)
    ...
    c=commands.add_parser("path",help="Find a local route; visible doors require --interact");select(c)
    ...
    c=commands.add_parser("validate",help="Validate native data and metadata; optionally flood-fill from a point");select(c)
    c.add_argument("--from",dest="start",nargs=2,type=int,help="Check every selected interaction/arrival is reachable")
    c=commands.add_parser("render",help="Render a PNG using actual game tiles and entity sprites");select(c)
...
def run(args):
    ...
    def render(path):
        ...
        m,bounds=selection()
        return Renderer(catalog).map(world,m,bounds,path,metadata,args.tile_size,args.grid,args.collision,args.labels,args.regions)
    ...
    if command=="catalog":
    ...
    elif command=="cell":result=cell(world,world.map(args.map),*args.at)
    elif command=="inspect":result=inspect(world,*selection())
    elif command=="space":
        m,bounds=selection()
        result=dict(map=m.id,rect=list(bounds),available=free_space(world,m,bounds,*args.size,args.limit,args.allow_regions))
    elif command=="path":
        m,bounds=selection()
        result=Navigation(world,m,bounds,args.cleared).path(args.start,args.to,args.interact)
        if not args.all_steps:result["path"]=corners(result["path"]);result["path_encoding"]="orthogonal corners"
        result.update(map=m.id,rect=list(bounds),cleared=args.cleared)
        result["revision"]=world.revision
        return result,0 if result["found"] else 1
    elif command=="validate":
        m,bounds=selection()
        result=validate(world,metadata,args.start,m.id,bounds)
        result["revision"]=world.revision
        ...
        validation=validate(world,metadata,requirements=requirements)
